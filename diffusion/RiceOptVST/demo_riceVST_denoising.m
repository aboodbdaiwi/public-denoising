...
z=sqrt((nu+sigma*randn(size(nu))).^2 + (sigma*randn(size(nu))).^2);   % raw magnitude MR data (noisy image)
...
sigma_hat = mean(std_noise_n_base);
%% denoising
...
Im = fz;
stdev = sigma_hat;            %the standard deviation of the noise

nslice = size(Im,3); % slice
matrix_size = size(Im);

% Denoising w/ GL-HOSVD
kglobal = .4; klocal = .8; %start: kglobal = 0.4; klocal = 0.8;
patchsize = 6;
step = 2;
sw = 12; % radius of search window

NImages_dnGL = zeros(matrix_size);
for i =1:nslice
    %                             for j = 1:Nbvalues
    tmp = (Im(:,:,i,:,:));
    NImages_dnGL(:,:,i,:,:) = glhosvd_flexible(tmp, stdev, kglobal, klocal, patchsize, step, sw);
    %                             end
end
